...
export function PaymentHistoryItem({ entry, scope = 'wage' }) {
  ...
  // Safe amount handling to completely prevent null .toLocaleString() crash
    ...
    <div className="flex items-center justify-between rounded-input border border-border px-4 py-3">
      <div>
        <div className="flex items-center gap-2">
          <p className="text-sm font-semibold text-text-primary">${safeAmount.toLocaleString()}</p>
          <Badge variant={entry?.type === 'Advance' ? 'warning' : 'success'}>{entry?.type || 'Payment'}</Badge>
        </div>
        {entry?.remarks && <p className="text-xs text-text-secondary mt-0.5">{entry.remarks}</p>}
      </div>
      <div className="flex items-center gap-3">
        <p className="text-xs text-text-secondary">{safeFormatDate(entry?.date)}</p>
